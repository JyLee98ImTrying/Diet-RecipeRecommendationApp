...
if page == "🍅🧀MyHealthMyFood🥑🥬":
    ...
    def handle_checkbox_change(idx, unique_key):
    ...
    def display_recommendations_with_selection(recommendations, key_prefix=''):
        ...
        for idx, row in recommendations.iterrows():
            ...
            with col1:
                st.checkbox("", key=unique_key, on_change=handle_checkbox_change, args=(idx, unique_key))
        ...
        plot_calories_breakdown(total_calories)
        display_nutritional_comparison(selected_recipes)
        compare_to_daily_intake(total_nutrients)
...
def plot_calories_breakdown(total_calories):
    labels = ['Selected Recipes', 'Remaining Daily Intake']
    values = [total_calories, DAILY_RECOMMENDED_INTAKE['Calories'] - total_calories]

    fig, ax = plt.subplots()
    ax.pie(values, labels=labels, autopct='%1.1f%%', colors=['skyblue', 'lightgrey'])
    ax.set_title('Calories Breakdown')
    st.pyplot(fig)

def display_nutritional_comparison(selected_recipes):
    if selected_recipes:
        st.write("### 🥗 Nutritional Comparison of Selected Recipes")
        selected_df = pd.DataFrame(selected_recipes)
        st.write(selected_df[['Name', 'Calories', 'ProteinContent', 'FatContent', 'CarbohydrateContent', 'SodiumContent', 'CholesterolContent', 'SaturatedFatContent', 'SugarContent']])

def compare_to_daily_intake(total_nutrients):
    st.write("### 📊 Comparison to Recommended Daily Intake")
    for nutrient, total_value in total_nutrients.items():
        daily_value = DAILY_RECOMMENDED_INTAKE.get(nutrient, None)
        if daily_value:
            percentage = (total_value / daily_value) * 100
            st.write(f"**{nutrient}:** {total_value:.1f}g ({percentage:.1f}% of daily intake)")
